...
def Video():
    ...
    while True:
        ...
        dim = (int(frame.shape[1] * r), 200)
        # perform the resizing
        resized = cv2.resize(frame, dim, interpolation=cv2.INTER_AREA)

        sub_image = resized[5:25, 30: 78]
        text = pytesseract.image_to_string(sub_image, config='--psm 10 --oem 3 -c tessedit_char_whitelist=.0123456789')
        temp = str(text[0:4])
        if flag:
            # annotated_frame = face_detector.draw(cv2.rotate(frame, cv2.ROTATE_180))
            annotated_frame = face_detector.draw(resized)
            if face_detector.detection_confidence > face_detector.confidence:
                f = open("data.txt", "w")
                str1 = "conf :" + "yes" + ": temp :" + str(temp)
                f.write(str1)
                f.close()
                print("Human Detected with confidence : ", "Yes", ": Body Temp : ", temp)
            else:
                f = open("data.txt", "w")
